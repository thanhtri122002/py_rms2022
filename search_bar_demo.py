...
def click():
    ...
    try:
        ...
        result.append('Cannot find that word in the list lma0')
...
# label
Label(window, text='Thầy Sơn ăn gì', font='none 12 bold').grid(row=0, column=0, sticky=W)

# text entry box
textbox = Entry(window, width=20, bg='grey', fg='white')
...
# text output box
output = Text(window, width=50, height=6, wrap=WORD, background='lightgrey')
...
# exit label
Label(window, text='Ấn ấn cái nút này để cái này này trở về nhà nhà', bg='white', font='none 9').grid(row=6, column=0, sticky=W)
